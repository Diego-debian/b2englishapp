...
| `slug` | String | Unique, Not Null | Identificador público URL-friendly; estrategia de rutas se define en fase FE. |
...
2.  **Block Order**:
    - Intención de unicidad compuesta: `Unique(lesson_id, order)`.
    - Determinismo: Si `order` no es estricto, la consulta debe ordenar secundariamente por `id ASC` para evitar saltos.
