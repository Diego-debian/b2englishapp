...
### Campos Comunes
- `id`: identificador interno, tipo no especificado en esta fase.
...
- **Renderizado**: Embed optimizado con lazy-load para performance; implementación se decide en fase frontend.

#### C. Block: `cta` (Call to Action)
- **Label**: Texto del botón (ej. "¡Practica el Past Simple ahora!").
- **Target Type**: `internal` | `external`.
- **Target**: String.
    - Si `internal`: ruta FE existente (ej. `/app/focus/past-simple`).
    - Si `external`: URL completa (ej. `https://google.com`).
