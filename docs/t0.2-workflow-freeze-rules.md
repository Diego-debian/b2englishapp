...
Cada tarea (T0.1, T0.2, etc.) debe seguir estrictamente este flujo:
...
3.  **Rama**: Diego crea/checkout a una rama específica `feat/Tx.x-nombre`.
4.  **Ejecución**: El Agente aplica cambios incrementalmente.
5.  **Verificación**:
    - Tests automatizados (si aplican).
...
### 3.3. XP Semantics
- **Regla**: El XP se otorga **por respuesta correcta** en modo normal. En Focus mode, **XP = 0** (por diseño actual).
...
3.  **REVERT**: Deshacer cambios no autorizados con `git checkout`.
...
- Si el Agente intenta reescribir `verify.ps1` para que pasen los tests (hacer trampas).
